...
def train(args, atkmodel, tgtmodel, clsmodel, device, train_loader,
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss2 = F.cross_entropy(tmpmodel(data), target_transform(target))
        loss2.backward()
        tgtoptimizer.step()
...
def test(args, atkmodel, scratchmodel, device, train_loader, test_loader, epoch,
    ...
    for _ in range(trainepoch):
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                loss2 = F.cross_entropy(output2, target_transform(target))
            loss.backward()
            testoptimizer.step()
            if batch_idx % args.log_interval * 100 == 0:
                print(
                    'Test_train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}\tTgtLoss: {:.6f}'.format(
                        epoch, batch_idx * len(data), len(train_loader.dataset),
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            test_transform_loss += F.cross_entropy(output,
                                                   target_transform(target),
                                                   reduction='sum').item()  # sum up batch loss
            pred = output.max(1, keepdim=True)[
                1]  # get the index of the max log-probability
            correct_transform += pred.eq(
                target_transform(target).view_as(pred)).sum().item()
    ...
    print(
        ...
        'Accuracy(Target): {}/{} ({:.0f}%)\n'.format(
            ...
            100. * correct_transform / len(test_loader.dataset),
        ))
    writer.add_scalar('acc(origin)', 100. * correct / len(test_loader.dataset),
                      global_step=epoch)
    writer.add_scalar('acc(target)',
                      100. * correct_transform / len(test_loader.dataset),
                      global_step=epoch)
    return 100. * correct_transform / len(test_loader.dataset)
...
def main():
    ...
    writer = SummaryWriter('train_targeted')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if (args.save_model):
            if acc > best_acc:
                best_acc = acc
                torch.save(atkmodel.state_dict(), "atkmnist_target_best.pt")
            torch.save(atkmodel.state_dict(), "atkmnist_target_latest.pt")
